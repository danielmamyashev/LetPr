...
|Барометр | BMP280|
|Инерционный датчик | MPU6050|
|Радиомодуль | HC-12|
|Буззер| FBMT9650S|
|Адаптер SD накопителя | Micro SD Adapter|
...
Структура программы в Arduino IDE состоит из двух функций:
...
> Датчик BMP280 измеряет температуру и давление. А по международной барометрической формуле мы можем вычислить высоту.
...
- Для вычисления высоты по международной барометрической формуле: ```pressureToAltitude()```
...
Для MPU6050 аналогично библиотеке BMP280 скачайте и добавте в скетч библиотеку [mpu6050](https://github.com/ElectronicCats/mpu6050).
...
Для работы радиомодуля используется протокол UART. Однако аппаратный UART также задействован для прошивки Arduino Nano по USB. Для того, чтобы избежать конфликта, создадим отдельный программный UART, подключив радиомодуль к пинам D5 и D6. 
...
```c++
SoftwareSerial HC12(6, 5); //Создание программного UART на пинах D5 и D6
```
...
<img src="s3.png" width="700"></img>

Проверим его содержимое:

<img src="s4.png" width="700"></img>
...
// Текущее время - Время в переменной = Прошеднее время
...
> **СПРАВОЧНАЯ ИНФОРМАЦИЯ**: ***Джампер*** - электрическая перемычка замыкающая контакты.
...
# Задание
## Написать программный код бортовой электроники в соответсвии с выбранным профилем.

## Условия
- Необходимо осуществлять передачу полетной телеметрии, включабщей в себя:
    * Значение давления;
    * Значение температуры;
    * Высоту полета над уровнем моря;
    * Значения акселерометра по трем осям;
    * Значения гироскопа по трем осям;
    * Углы ориентации ракеты;
    * Значения акселерометра по трем осям;
    * Флаг текущего этапа полета.
- Необходимо производить запить данных полетной телеметрии на энергонезависимую память;
