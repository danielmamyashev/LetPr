...
Для работы радиомодуля используется протокол UART. Однако аппаратный UART также задействован для прошивки Arduino Nano по USB. Для того, чтобы избежать конфликта, создадим отдельный программный UART, подключив радиомодуль к пинам D7 и D6. 
...
```c++
SoftwareSerial HC12(7, 6); //Создание программного UART на пинах D7(RX) и D6(TX)
```
...
Как можно заметить по схеме, в качестве SS(CS) пина мы используем пин A0. Однако по умолчанию в Arduino Nano под SS(CS) выделен пин D10. Мы будем использовать его в дальнейшем. Потому, дабы избежать конфликта, пропишем в секции setup():
```c++
pinMode(10, OUTPUT);
```
...
Схема подключения (буззер на плате):

<img src="connection_scheme_buzzer.png" width="500">

Схема подключения (буззер без платы. Требуется отдельный транзистор):

<img src="connection_scheme_buzzer_2.png">
